...
// Version 1.13 - 25-Dec-2013 - fix for broken image issue with full NNVL URLs
// Version 1.14 - 28-Dec-2013 - fix for locating hiRes image
//
// Usage:
//
...
$Debug = "get-nnvl-iod.php V1.14 - 28-Dec-2013<br/>\n";
...
if(preg_match('|Transfer-Encoding:\s+chunked|i',$rawhtml)) {
	$Debug .= "chunked response, un-chunking\n";
    $data = substr($rawhtml, (strpos($rawhtml, "\r\n\r\n")+4));
	$Debug .= "in=".strlen($data);
    $data = unchunkIOD($data);
	$Debug .= " bytes, unchunked=".strlen($data)." bytes\n";
	$html = $data;
//	$tfile = preg_replace('|\.txt$|','-unchunk.txt',"$IODdir$IODpageCache");
//	$fp = fopen($tfile,'w');
//	if ($fp) {
//		$write = fputs($fp,$data);
//		fclose($fp);
//		$Debug .= "debug cache written to $tfile\n";
//	}
}
...
preg_match_all('|<td colspan=\'2\'>(.+)</td>|Uis',$html,$matches);
    $Debug .= "main matches:".print_r($matches,true);
...
New version:
     ...
     [0] => Array 
         ( 
             [0] => <td colspan='2'><a class='pointer' onclick='shadowboxopen("images/high_resolution/1476v1_20131220-US.png", "Storm System to Bring Hazards to Several Parts of the Nation through this Weekend" )'><img style='height:350px; width:620px;' class='imageMediaPic' src='http://www.nnvl.noaa.gov/images/low_resolution/620x350x1476v1_20131220-US.png.pagespeed.ic.iowPfkKoh2.jpg' alt='Storm System to Bring Hazards to Several Parts of the Nation through this Weekend'/></a></td> 
             [1] => <td colspan='2'></td></tr></table>			 
 </td> 
             [2] => <td colspan='2'><table><tr><td colspan='2'>Dec 20, 2013</td> 
             [3] => <td colspan='2'><div id='mediaPageTitle2'><big><big>Storm System to Bring Hazards to Several Parts of the Nation through this Weekend</big></big></div></td> 
             [4] => <td colspan='2'><p>A potent storm system will track across the nation through this weekend bringing a variety of weather hazards and impacting holiday travel for much of the country. As the system moves into the Plains, it will be able to draw additional moisture from the Gulf of Mexico increasing the effects of the storm. Heavy rain is forecast from the lower Mississippi River Valley to the Ohio Valley this weekend with a risk for both areal and flash flooding. A wintry mix, including freezing rain and snow, is possible from the central Great Plains, through the Great Lakes and to northern New England this weekend. The NWS Storm Prediction Center (SPC) has indicated a Moderate Risk of severe thunderstorms on Saturday, with possible tornadoes, for portions of the Lower Mississippi Valley. Severe thunderstorms are possible from the Central Gulf Coast/Lower Mississippi Valley into the Ohio Valley Saturday. Unseasonable (perhaps record) warmth continues to build into the southern and eastern U.S. ahead of this storm system, but a powerful cold front will sweep through the Eastern Seaboard bringing a return to normal winter temperatures by Monday. Heavy snow is also forecast for the higher elevations in the Pacific Northwest and Northern Rockies into the weekend. This image was taken by GOES East at 1745Z on December 20, 2013.</p></td> 
             [5] => <td colspan='2'><div class='title1'><div class='title2' align='left' style='padding-left: 20px;'></div></div></td> 
         ) 
  
     [1] => Array 
         ( 
             [0] => <a class='pointer' onclick='shadowboxopen("images/high_resolution/1476v1_20131220-US.png", "Storm System to Bring Hazards to Several Parts of the Nation through this Weekend" )'><img style='height:350px; width:620px;' class='imageMediaPic' src='http://www.nnvl.noaa.gov/images/low_resolution/620x350x1476v1_20131220-US.png.pagespeed.ic.iowPfkKoh2.jpg' alt='Storm System to Bring Hazards to Several Parts of the Nation through this Weekend'/></a> 
             [1] => </td></tr></table>			 
  
             [2] => <table><tr><td colspan='2'>Dec 20, 2013 
             [3] => <div id='mediaPageTitle2'><big><big>Storm System to Bring Hazards to Several Parts of the Nation through this Weekend</big></big></div> 
             [4] => <p>A potent storm system will track across the nation through this weekend bringing a variety of weather hazards and impacting holiday travel for much of the country. As the system moves into the Plains, it will be able to draw additional moisture from the Gulf of Mexico increasing the effects of the storm. Heavy rain is forecast from the lower Mississippi River Valley to the Ohio Valley this weekend with a risk for both areal and flash flooding. A wintry mix, including freezing rain and snow, is possible from the central Great Plains, through the Great Lakes and to northern New England this weekend. The NWS Storm Prediction Center (SPC) has indicated a Moderate Risk of severe thunderstorms on Saturday, with possible tornadoes, for portions of the Lower Mississippi Valley. Severe thunderstorms are possible from the Central Gulf Coast/Lower Mississippi Valley into the Ohio Valley Saturday. Unseasonable (perhaps record) warmth continues to build into the southern and eastern U.S. ahead of this storm system, but a powerful cold front will sweep through the Eastern Seaboard bringing a return to normal winter temperatures by Monday. Heavy snow is also forecast for the higher elevations in the Pacific Northwest and Northern Rockies into the weekend. This image was taken by GOES East at 1745Z on December 20, 2013.</p> 
             [5] => <div class='title1'><div class='title2' align='left' style='padding-left: 20px;'></div></div> 
         ) 
...
if(isset($matches[1])) {
   ...
   preg_match('!shadowboxopen\("([^"]+)"!is',$matches[1][0],$mtemp);
   $Debug .= "new hires match: ".print_r($mtemp,true)."<br/>\n";
   if(isset($mtemp[1]) and strlen($IODimageHiRes) < 1) {
	   $IODimageHiRes = $mtemp[1];
	   if(strpos($IODimageHiRes,$IODURL) == false) {
		   $IODimageHiRes = $IODURL . $IODimageHiRes;
	   }
	   $Debug .= "hiRes: '$IODimageHiRes'<br/>\n";
	   $IODimageHiRes = "<a href=\"$IODimageHiRes\">High Resolution Image</a>";
   }
	if(preg_match('|<img.*src=\'([^\']+)\'|Uis',$rawImg,$tmatch)) {
		$IODimageURL = $tmatch[1];
		if(!preg_match('|http://|',$IODimageURL)) { $IODimageURL = $IODURL . $IODimageURL; }
...
if(isset($IODimageURL) and ! preg_match('|\.gif$|i',$IODimageURL)) {
// get dates
...
//	print_r($Headers);
    ...
    if ($doRefresh or $GraphicTime > $CacheTime) {
	  ...
	  if(preg_match('|\.gif$|i',$IODimageURL)) {
         $image = loadGIF($IODdir.$IODimageCacheFS);  // fetch our full sized image
	  }
	  $MaxX = imagesx($image);
	  $MaxY = imagesy($image);
	...
	$sorryMsg = '<h3>Sorry... today\'s image is a movie or animated .gif file and cannot be reformatted to show here. Please visit the <a href="'.$IODpageURL.'">NNVL site</a> to view it.</h3>';
}
...
if (preg_match('|^<p>|i',$IODimageText)) {
	print "$IODimageText\n";
} else {
